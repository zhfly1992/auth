...
package com.run.authz.api.base.util;
...
public class ExceptionChecked <T>{
	...
	public static <T> RpcResponse<T> checkRequestParam(String requestJson) {
		try {
			// 判断是否为空
			if (ParamChecker.isBlank(requestJson)) {
				return RpcResponseBuilder.buildErrorRpcResp("传入参数为空！");
	...
	public static <T> RpcResponse<T> checkRequestKey(Logger logger, String methodName, JSONObject json,
		...
		for (String key : keys) {
			if (isEmpty(json.getString(key))) {
				logger.error(String.format("[%s()->error:%s:%s]", methodName, AuthzConstants.NO_BUSINESS, key));
